        ...
        'locale' => [
            ...
            'props'     => [
                'plugin' => (static function () {
                    return function ($plugin = null): string {
                        if (is_string($plugin)) {
                            $trimmed = trim($plugin);
                ...
                'reset' => (static function () {
                    return function ($reset = null): bool {
                        return $reset === null ? true : (bool) $reset;
                    };
                })(),
                'empty' => (static function () {
                    return function ($empty = null): array {
                        if (is_string($empty)) {
                            $text = trim($empty);
        ...
        'site' => function (App $kirby) use (
            ...
            if ($allowedTemplates !== [] && is_array($pageCreate) && isset($pageCreate['load']) && is_callable($pageCreate['load'])) {
                ...
                $dialogs['page.create'] = array_replace($pageCreate, [
                    'load' => function () use ($kirby, $load, $extendDialogWithLocaleField, $extractLocaleFromRequest, $normaliseTitleLocale, $fieldOverrides) {
                        $dialog  = $load();
                        $request = $kirby->request();
                        [$hasValue, $rawValue] = $extractLocaleFromRequest($request);
